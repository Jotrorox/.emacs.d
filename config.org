...
#+Date: 2024 - today
Welcome! This is my small Emacs Config mainly written in org based on
the awesome [[https://github.com/ianyepan/yay-evil-emacs][yay-evil]] Config by Ian. I tried to keep all my added code
...
** Quelpa
Quelpa is a simple package manager for packages from all sources
including GitHub, which we'll need later on, since not all the
packages are available in Melpa yet.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
*** LSP Mode
LSPs are a central Part of modern Development because
they provide a poweful way to autocomplete and see
error ahead of runtime or compilation. LSP Mode is the
Main way of using LSPs in Emacs so we'll set that up here.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
*** Languages
Here I keep a lot of my per Language Configurations for more info
please look into the config sections themselfs.
**** C3
C3 is a simple and new language iteration on top of C improving
a lot of the concepts and adding a few new ones. [[https://c3-lang.org][C3 Website]]

First we'll install the Treesitter grammar, which is used for
highlighting. Note that this needs `cc` to be in the path.
#+BEGIN_SRC emacs-lisp
  (setq treesit-language-source-alist
        '((c3 "https://github.com/c3lang/tree-sitter-c3")))
#+END_SRC

Now we'll install the main Plugin through quelpa, since it isn't
on Melpa yet. The Pugin provides a Major mode for c3 and a few
...
#+END_SRC
**** Nix
Nix is a powerful functional programming Lanugage that is used
to configure nix-envs aswell as the Nix Operating System. Here
we will use the official nix-mode provided by NixOS.
#+BEGIN_SRC emacs-lisp
  (use-package nix-mode :ensure t)
#+END_SRC
*** GitHub Copilot
One of the best autocompletion AI integrations in my opinion. It
is not officially supported in Emacs and needs node installed.

Setup Install Copilot using Quelpa since it is not available in the
...
#+END_SRC
*** XKCD
XKCD is an awesome webcomic that can also be viewed in a emacs
buffer using a plugin, that is everything I'll do here...
#+BEGIN_SRC emacs-lisp
  (use-package xkcd :ensure t)
#+END_SRC
*** Zen Mode
ZenMode Focuses on the Code and is awesome for showcases and
Tutorials.
#+BEGIN_SRC emacs-lisp
  (use-package zen-mode :ensure t)
